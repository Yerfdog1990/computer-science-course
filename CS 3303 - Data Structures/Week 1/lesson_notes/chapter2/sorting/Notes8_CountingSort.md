...
Counting Sort relies on counting occurrences of distinct values, so they must be integers. With integers, each value fits with an index (for non negative values), and there is a limited number of different values, so that the number of possible different values
*k*
...
If the number of possible different values to be sorted
*k*
...
After counting a value, we can remove it, and count the next value, which is 3.
...
The next value we count is 0, so we increment index 0 in the counting array.
...
With this in mind, we can start implementing the algorithm using Java.
...
public class CountingSort {
    ...
    public static void main(String[] args) {
        int[] unsortedArr = {4, 2, 2, 6, 3, 3, 1, 6, 5, 2, 3};
...
* **Worst case:**
  ...
  *n*,
  Counting Sort can have time complexity:
  [
  O(n^2)
  ]
  or even worse.
...
O(n²)
(red line) and best case scenario
O(n)
(green line).

---

As mentioned previously: if the numbers to be sorted varies a lot in value (large
*k*),
and there are few numbers to sort (small
*n*),
...
and
*k*
fixed, the "Random", "Descending" and "Ascending" alternatives in the simulation above result in the same number of operations. This is because the same thing happens in all three cases: A counting array is set up, the numbers are counted, and the new sorted array is created.
